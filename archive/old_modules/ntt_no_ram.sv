module ntt_no_rams (
  ...
  */
  ...
  for (index = 0; index < 4; index++) assign test_zeta[index] = zetas[k+index];
  ...
  always @(posedge clk) begin
    if (reset) begin
      ...
      if (step == load) begin
        ...
        end  // compute_count 1,2,3,4 use for cooley_tookes's fqmul to compute
             // using 4 clk cycle
        else if (compute_count < 5) begin
          compute_count <= compute_count + 1;
          fqmul_start   <= 0;
        end  // compute_count = 5 : loading value to buf_out
        else if (compute_count == 5) begin
          compute_count <= 0;
          // use this formular for the round that len < 8
          // stage 0 : len = 128;
